...
1. finish source acquisition parity for unstable real-network candidates
2. add downloader handshake phase evidence to the real-network parity scenario
3. lift queue-only and late-accept behavior to scenario-level parity
4. extend callback parity beyond request issuance into callback-session startup
5. add listener and upload-serving parity scenarios in both plaintext and obfuscated modes
